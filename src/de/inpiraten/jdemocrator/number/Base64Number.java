 ...
 */
package de.inpiraten.jdemocrator.number;
...
public class Base64Number {
	...
	/**
	 * Converts a binaryString to a corresponding Base64 String
	 * @param binaryString
	 * @return
	 */
	public static String toBase64String(String binaryString){
		String output = "";
		...
		while (binaryString.length()>0){
			...
			String currentDigit = binaryString.substring(binaryString.length()-6, binaryString.length());
			
			byte currentDigitValue = (byte) Integer.parseInt(currentDigit,2);
				
			try {
				output = toDigit(currentDigitValue)+output;
			} catch (NoBase64NumberException e) {
				throw new RuntimeException("Binary digit value to high. This should not have happend!");
			}
			
			binaryString = binaryString.substring(0, binaryString.length()-6);
